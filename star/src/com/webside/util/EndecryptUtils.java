...
public class EndecryptUtils {
    ...
    public static MemberEntity md5MemberPassword(String mAccountName,String password,int hashIterations){ 
        SecureRandomNumberGenerator secureRandomNumberGenerator=new SecureRandomNumberGenerator(); 
        String salt= secureRandomNumberGenerator.nextBytes().toHex(); 
        String password_cryto = new Md5Hash(password,mAccountName+salt,hashIterations).toBase64(); 
        MemberEntity member=new MemberEntity(); 
        member.setmPassword(password_cryto); 
        member.setmCredentialsSalt(salt); 
        member.setmAccountName(mAccountName);
        return member; 
    } 
